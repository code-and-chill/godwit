...
class ChatState extends AppState {
  ...
  void getUserchatList(String userId) {
    try {
      kDatabase.child('chatUsers').child(userId).once().then((DataSnapshot snapshot) {
  ...
  void getchatDetailAsync() async {
    try {
      kDatabase.child('chats').child(_channelName).once().then((DataSnapshot snapshot) {
  ...
  void _onMessageAdded(Event event) {
    ...
    if (event.snapshot.value != null) {
      ...
      if (map != null) {
        ...
        if (_messageList.length > 0 && _messageList.any((x) => x.key == model.key)) {
          return;
        }
        _messageList.add(model);
      }
