...
class ChatState extends AppState {
  ...
  void databaseInit(String userId, String myId) async {
    ...
    if (messageQuery == null || _channelName != getChannelName(userId, myId)) {
      messageQuery = firebaseDatabase.child("chats").child(_channelName);
  ...
  ///  } ```
  /// For more detail visit:- https://github.com/TheAlphamerc/twitter/issues/28#issue-611695533
  ...
  void getUserchatList(String userId) {
    try {
      firebaseDatabase.child('chatUsers').child(userId).once().then((
          DataSnapshot snapshot) {
        _chatUserList = List<Message>();
        if (snapshot.value != null) {
          var map = snapshot.value;
          if (map != null) {
            map.forEach((key, value) {
              var model = Message.fromJson(value);
              model.key = key;
  ...
  void getchatDetailAsync() async {
    try {
      firebaseDatabase.child('chats').child(_channelName).once().then((
          DataSnapshot snapshot) {
  ...
  void onMessageSubmitted(Message message, {User myUser, User secondUser}) {
    ...
    try {
      ...
      firebaseDatabase.child('chatUsers').child(message.senderId).child(
          message.receiverId).set(message.toJson());

      firebaseDatabase.child('chatUsers').child(chatUser.userId).child(
          message.senderId).set(message.toJson());

      firebaseDatabase.child('chats').child(_channelName).push().set(
          message.toJson());
  ...
  void _onMessageAdded(Event event) {
    ...
    if (event.snapshot.value != null) {
      ...
      if (map != null) {
        var model = Message.fromJson(map);
  ...
  void sendAndRetrieveMessage(Message model) async {
    /// on noti
